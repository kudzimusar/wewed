...
const mergeReviewMigration = read('prisma/migrations/20260824031200_booking_merge_review_fixes/migration.sql')
...
requireText(governance, 'header.deliveryAt ?? eventStart', 'Governed allocation must include delivery time before falling back to the event day.')
requireText(governance, 'header.collectionAt ?? (header.appointmentAt', 'Governed allocation must include collection time before appointment/event-day fallback.')
requireText(governance, 'SELECT id,"bookingId",status,"expiresAt" FROM wewed_booking."BookingHold"', 'Hold idempotency lookup must bind the key to its booking.')
requireText(governance, 'IDEMPOTENCY_KEY_REUSED', 'Cross-booking hold idempotency reuse must fail closed.')
...
requireText(mergeReviewMigration, 'original."entryType"=\'REFUND\' THEN m.amount', 'A reversal of a refund must restore paid value rather than subtract it again.')
...
requireText(postgresContract, 'refund reversal sign contract failed', 'PostgreSQL contract must prove reversal-of-refund restores deposit truth.')
