...
assert(billingPortal.includes('— cancellation scheduled'), 'Billing must state scheduled cancellation directly in the subscription status.')
assert(billingPortal.includes("cancellationScheduled ? 'Access until' : 'Next renewal'"), 'Billing must distinguish cancellation access from renewal dates.')
assert(billingPortal.includes('will not renew'), 'Billing must clearly warn customers that a scheduled cancellation stops renewal.')
assert(billingPortal.includes("timeZone: 'UTC'"), 'Billing dates must not shift by the browser timezone.')
...
assert(billingRoute.includes('cancelAtPeriodEnd: metadataBoolean'), 'Billing account API must expose scheduled cancellation state.')
...
assert(billingSyncRoute.includes('cancel_at_period_end'), 'Reconciliation must retrieve Stripe period-end cancellation state.')
assert(billingSyncRoute.includes('cancel_at?: number | null'), 'Reconciliation must also support Stripe scheduled cancellation timestamps.')
assert(billingSyncRoute.includes('scheduledCancellationAt !== null'), 'Reconciliation must treat an explicit Stripe cancellation timestamp as scheduled cancellation.')
assert(billingSyncRoute.includes('[keys.cancelAtPeriodEnd]'), 'Reconciliation must persist scheduled cancellation state.')
...
assert(stripeBilling.includes('cancelAtPeriodEnd: `${prefix}CancelAtPeriodEnd`'), 'Stripe metadata must namespace scheduled cancellation state.')
...
assert(webhookRoute.includes('object.cancel_at_period_end'), 'Webhooks must persist Stripe period-end cancellation changes.')
assert(webhookRoute.includes('integer(object.cancel_at) !== null'), 'Webhooks must also detect explicit Stripe cancellation timestamps.')
assert(webhookRoute.includes('subscriptionCancellationScheduled'), 'Webhook cancellation handling must use one canonical detector.')
