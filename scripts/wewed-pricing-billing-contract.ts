...
assert(billingPortal.includes('aria-label="Subscription overview"'), 'Billing must expose one semantic subscription overview.')
assert(billingPortal.includes('grid grid-cols-2 lg:grid-cols-4'), 'Subscription metrics must stay in a compact 2x2 phone grid and four-column wide layout.')
assert(billingPortal.includes('currentPlanName'), 'Billing must display the public plan name instead of the internal plan ID.')
assert(!billingPortal.includes('grid gap-4 md:grid-cols-4'), 'Billing must not regress to four oversized standalone metric cards.')
