...
export async function resetMarketplaceE2EFixture(): Promise<void> {
  ...
  try {
    await prisma.weddingMembership.delete({ where: { userId_weddingId: { userId: E2E_USER.id, weddingId: E2E_WEDDINGS.primary.id } } })
    await prisma.user.update({ where: { id: E2E_USER.id }, data: { currentWeddingId: E2E_WEDDINGS.secondary.id } })
    await prisma.user.create({ data: { id: E2E_COUPLE.id, email: E2E_COUPLE.email, name: E2E_COUPLE.name, role: 'couple', coupleId: E2E_WEDDINGS.primary.coupleId, currentWeddingId: E2E_WEDDINGS.primary.id, isActive: true } })
    await prisma.weddingMembership.create({ data: { id: 'e2e-market-couple-owner-membership', userId: E2E_COUPLE.id, weddingId: E2E_WEDDINGS.primary.id, role: 'owner', status: 'active', acceptedAt: new Date() } })

    await prisma.$executeRawUnsafe(`DELETE FROM wewed_admin."BusinessAccount" WHERE id IN ('${E2E_MARKETPLACE.plannerBusinessId}','${E2E_MARKETPLACE.coupleBusinessId}')`)
    await prisma.$executeRawUnsafe(`INSERT INTO wewed_admin."BusinessAccount" (id,name,slug,type,status,"ownerUserId","onboardingStatus","subscriptionPlan","subscriptionStatus",metadata,"createdAt","updatedAt") VALUES
    ...
    await prisma.$executeRawUnsafe(`INSERT INTO wewed_admin."BusinessAccountMember" (id,"businessAccountId","userId",role,status,permissions,"createdAt","updatedAt") VALUES
    ...
    await prisma.$executeRawUnsafe(`INSERT INTO wewed_admin."BusinessAccountLink" (id,"businessAccountId","entityType","entityId",relationship,"createdAt") VALUES
    ...
    await prisma.$executeRawUnsafe(`INSERT INTO wewed_admin."PlannerProfile" (id,"businessAccountId",slug,"displayName",headline,bio,"yearsExperience","serviceAreas",services,"weddingStyles",languages,"priceBand","availabilityStatus",status,"publishedAt","createdAt","updatedAt") VALUES
...
export async function marketplaceMembershipStatus(): Promise<string | null> {
  const prisma = new PrismaClient()
  try { const row = await prisma.weddingMembership.findUnique({ where: { userId_weddingId: { userId: E2E_USER.id, weddingId: E2E_WEDDINGS.primary.id } }, select: { status: true } }); return row?.status ?? null } finally { await prisma.$disconnect() }
}
