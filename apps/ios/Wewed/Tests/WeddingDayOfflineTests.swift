...
final class WeddingDayOfflineTests: XCTestCase {
    ...
    func testRevokePassCallsRevokeEndpoint() async throws {
        ...
        let result = await service.revokePass(
        ...
        for forbidden in ["weddingId", "gateId", "usherId", "operatorUserId"] {
            XCTAssertNil(json[forbidden])
        }
    }

    func testRevokePassPreservesServerFailureReason() async {
        RevokeStubProtocol.lastRequest = nil
        RevokeStubProtocol.statusCode = 403
        RevokeStubProtocol.responseBody = "{\"success\":false,\"code\":\"GATE_GRANT_REVOKED\",\"error\":\"This gate assignment is no longer authorized.\"}"
        let config = URLSessionConfiguration.ephemeral
        config.protocolClasses = [RevokeStubProtocol.self]
        let service = WeddingDaySyncService(session: URLSession(configuration: config))
        ...
        XCTAssertEqual(result.code, "GATE_GRANT_REVOKED")
        XCTAssertEqual(result.error, "This gate assignment is no longer authorized.")
    }

    func testLocallyMarkedRevocationFailsClosedOffline() async throws {
        let store = OfflineManifestStore(storageDirectory: nil)
        try await store.saveManifest(
