...
- `UserProfile.id` **is** the verified Supabase auth user id. There is no separate foreign-key column; the primary key itself is the Supabase uid. Every onboarding writer (`/api/auth/register`, `/api/admin/onboarding`) sets it explicitly at creation (`userProfile.create({ data: { id: authUserId, ... } })` / `userProfile.upsert({ where: { id: authUserId }, ... })`) — never inferred after the fact.
...
Every other write in both branches was already idempotent before this phase (`WeddingMembership` keyed on `@@unique([userId, weddingId])`, `BusinessAccountLink` via `ON CONFLICT ... DO UPDATE`, `UserProfile` via `upsert` on its primary key) and is unchanged.
