...
function QuickStartContent() {
  ...
  async function save(formElement: HTMLFormElement, submitForReview: boolean) {
    const form = new FormData(formElement)
    const serviceAreas = form.getAll('serviceAreas').map(String)
    const services = form.getAll('services').map(String)
    const bio = String(form.get('bio') || '').trim()

    if (submitForReview && (!bio || serviceAreas.length === 0 || services.length === 0)) {
      setError('Before review, add a short biography, at least one service area, and at least one service.')
      return
    }
  ...
  >
    {loading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="size-6 animate-spin" /></div> : <>
      ...
      {!lockedForReview && !published && !suspended && <form key={String(profile?.id ?? 'quick-start')} onSubmit={(event) => { event.preventDefault(); void save(event.currentTarget, false) }} className="space-y-5">
        <Card className="border-gold/20 bg-champagne"><CardContent className="space-y-5 p-6">
          ...
          <div><label className="text-sm font-medium text-espresso">Short biography</label><Textarea name="bio" defaultValue={String(profile?.bio ?? '')} placeholder="Tell couples what you plan, where you work and what they can expect from you. Two or three sentences is enough to start." className="mt-2 min-h-28 bg-white" /></div>
        </CardContent></Card>

        <Card className="border-gold/20 bg-champagne"><CardContent className="p-6"><fieldset><legend className="text-sm font-medium text-espresso">Where do you work?</legend><p className="mt-1 text-xs text-espresso/50">Choose at least one before review. You can add more later.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{SERVICE_AREAS.map((area) => <label key={area} className="flex items-center gap-2 rounded-lg border border-gold/15 bg-white/70 px-3 py-2 text-xs text-espresso"><input name="serviceAreas" type="checkbox" value={area} defaultChecked={selectedAreas.includes(area)} className="accent-[#BF9B5F]" />{area}</label>)}</div></fieldset></CardContent></Card>

        <Card className="border-gold/20 bg-champagne"><CardContent className="p-6"><fieldset><legend className="text-sm font-medium text-espresso">What do you help couples with?</legend><p className="mt-1 text-xs text-espresso/50">Choose at least one service before review. You can expand the list later.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{SERVICES.map((service) => <label key={service} className="flex items-center gap-2 rounded-lg border border-gold/15 bg-white/70 px-3 py-2 text-xs text-espresso"><input name="services" type="checkbox" value={service} defaultChecked={selectedServices.includes(service)} className="accent-[#BF9B5F]" />{service}</label>)}</div></fieldset></CardContent></Card>

        <div className="flex flex-col gap-3 rounded-2xl border border-gold/20 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-espresso">You can stop and save at any point.</p><p className="mt-1 text-xs text-espresso/50">The four essentials are required only when you send the profile to Wewed for review.</p></div><div className="flex flex-wrap gap-2"><Button type="submit" variant="outline" disabled={busy !== null}><Save className="size-4" />{busy === 'save' ? 'Saving…' : 'Save draft'}</Button><Button type="button" disabled={busy !== null} className="bg-espresso text-champagne" onClick={(event) => { const form = event.currentTarget.closest('form'); if (form instanceof HTMLFormElement) void save(form, true) }}><CheckCircle2 className="size-4" />{busy === 'submit' ? 'Submitting…' : 'Save & submit for review'}</Button></div></div>
      </form>}
    </>}
  </MarketplaceFrame>
}
