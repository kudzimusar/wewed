...
import { BriefcaseBusiness, ClipboardList, House, LayoutDashboard, MessageCircle, Sparkles } from 'lucide-react'
...
export function PlannerAccountDock() {
  ...
  return (
    <nav
      className="fixed bottom-3 left-1/2 z-[180] flex max-w-[calc(100vw-1rem)] -translate-x-1/2 items-center gap-1 overflow-x-auto rounded-full border border-gold/25 bg-espresso/95 p-1.5 text-champagne shadow-2xl backdrop-blur"
      ...
      {items.map(([href, label, Icon]) => {
        ...
        return (
          <Link
            ...
            className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-xs font-semibold ${active ? 'bg-gold text-espresso' : 'text-champagne/70 hover:bg-gold/10 hover:text-gold'}`}
          >
