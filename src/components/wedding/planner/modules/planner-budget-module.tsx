...
interface BudgetForm { description: string; category: string; estimatedCost: string; actualCost: string; paidAmount: string; vendorName: string; notes: string; dueDate: string }
interface PlannerBudgetModuleProps {
  budget: BudgetRow[]; budgetSummary: BudgetSummary | null; budgetByCategory: CategoryBreakdown[]; budgetForm: BudgetForm; setBudgetForm: Dispatch<SetStateAction<BudgetForm>>; saving: boolean
...
export function PlannerBudgetModule({ budget, budgetSummary, budgetByCategory, budgetForm, setBudgetForm, saving, onAddBudgetItem, onUpdateBudgetItem, onDeleteBudgetItem }: PlannerBudgetModuleProps) {
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Estimated', budgetSummary?.totalEstimated ?? 0], ['Actual', budgetSummary?.totalActual ?? 0], ['Paid', budgetSummary?.totalPaid ?? 0], ['Outstanding', budgetSummary?.totalOutstanding ?? 0]].map(([label, value]) => <SectionCard key={String(label)} className="p-4"><p className="font-sans text-[10px] uppercase tracking-[0.16em] text-gold/75">{label}</p><p className="mt-2 font-serif text-2xl">{money(Number(value), budgetSummary?.currency)}</p></SectionCard>)}</div>
    {budgetSummary && <SectionCard className="p-4"><div className="flex items-center justify-between gap-3 font-sans text-xs text-champagne/60"><span>Payment progress</span><span className="text-gold">{budgetSummary.percentPaid}% paid</span></div><Progress value={budgetSummary.percentPaid} className="mt-2 h-1.5 bg-champagne/10 [&>div]:bg-gold" /></SectionCard>}
    ...
    <SectionCard className="p-4"><form onSubmit={onAddBudgetItem} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <div><Label htmlFor="workspace-budget-vendor">Vendor</Label><Input id="workspace-budget-vendor" value={budgetForm.vendorName} onChange={(event) => setBudgetForm((current) => ({ ...current, vendorName: event.target.value }))} className="mt-1 border-gold/20 bg-espresso/70" placeholder="Imba Manor" /></div>
