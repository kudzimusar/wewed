  ...
  return (
    <div className="min-h-screen bg-ivory text-espresso" data-release="wedding-first-v2">
          ...
          <div className="flex items-center gap-2">
            ...
            <details className="relative xl:hidden">
              ...
              <div className="absolute right-0 top-12 z-50 w-64 rounded-2xl border border-gold/20 bg-espresso p-3 shadow-2xl">
                ...
                <Link href="/legal" className="block rounded-xl px-3 py-2.5 text-sm text-champagne/80 hover:bg-gold/10 hover:text-gold">Legal center</Link>
                <Link href="/developers" className="block rounded-xl px-3 py-2.5 text-sm text-champagne/80 hover:bg-gold/10 hover:text-gold">Developers</Link>
      ...
      <footer className="border-t border-gold/20 bg-espresso text-champagne">
        <div className="mx-auto grid max-w-[90rem] gap-10 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-[1.4fr_1fr_1fr_1fr_1fr] lg:px-8">
          <div><p className="flex items-center gap-2 font-serif text-3xl text-gold"><HeartHandshake className="size-5" />wewed</p><p className="mt-3 max-w-sm text-sm leading-6 text-champagne/60">Private wedding planning, professional support and memorable guest experiences in one connected platform.</p><p className="mt-5 text-xs text-champagne/45">wewed.pro · Privacy-led wedding technology.</p></div>
          ...
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">Discover</p><div className="mt-4 flex flex-col gap-2 text-sm text-champagne/70">{PUBLIC_LINKS.slice(3, 6).map(([label, href]) => <Link key={href} href={href} className="hover:text-gold">{label}</Link>)}</div></div>
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">Trust & legal</p><div className="mt-4 flex flex-col gap-2 text-sm text-champagne/70">{TRUST_LINKS.map(([label, href]) => <Link key={href} href={href} className="hover:text-gold">{label}</Link>)}</div></div>
        ...
        <div className="border-t border-gold/10 px-4 py-5 text-center text-xs text-champagne/40">© {new Date().getFullYear()} Wewed · wewed.pro · Privacy-led wedding technology.</div>
      </footer>
    </div>
  )
}
