...
export function AdminUtilityNav() {
  ...
  return (
    <nav className="fixed bottom-4 left-4 z-50 flex items-center gap-2 rounded-full border border-gold/25 bg-espresso/95 p-1.5 shadow-2xl backdrop-blur" aria-label="Wewed administrator navigation">
      ...
      <a href="/admin/onboarding" className={`inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold ${pathname.startsWith('/admin/onboarding') ? 'bg-gold text-espresso' : 'text-gold hover:bg-gold/10'}`}>
        <UserRoundCheck className="size-4" />Onboarding
      </a>
