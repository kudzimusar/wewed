...
type Item = {
  ...
  serviceAreaPolicy?: Record<string, unknown>
}
...
function policyTerms(value: Record<string, unknown> | undefined) {
  const terms = value?.allowedTerms
  return Array.isArray(terms) ? terms.filter((entry): entry is string => typeof entry === 'string').join(', ') : ''
}
...
export default function VendorAvailabilityPage() {
  ...
  const [policy, setPolicy] = useState({ timezone:'Africa/Harare',minNoticeMinutes:'',bookingHorizonDays:'',minDurationMinutes:'',maxDurationMinutes:'',serviceAreas:'',serviceAreaRequired:false })
  ...
  const componentCandidate = useMemo(() => candidates.find((item) => item.id === component.childCatalogItemId) || null, [candidates,component.childCatalogItemId])
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      setItems(nextItems)
      setItemId((current) => nextItems.some((item) => item.id === current) ? current : nextItems[0]?.id || '')
  ...
  const loadItem = useCallback(async (targetId: string) => {
    ...
    try {
      ...
      const area = current?.serviceAreaPolicy || {}
      setPolicy({
        timezone: current?.operatingTimezone || 'Africa/Harare',
        minNoticeMinutes: current?.minNoticeMinutes == null ? '' : String(current.minNoticeMinutes),
        bookingHorizonDays: current?.bookingHorizonDays == null ? '' : String(current.bookingHorizonDays),
        minDurationMinutes: current?.minDurationMinutes == null ? '' : String(current.minDurationMinutes),
        maxDurationMinutes: current?.maxDurationMinutes == null ? '' : String(current.maxDurationMinutes),
        serviceAreas: policyTerms(area),
        serviceAreaRequired: area.required === true,
      })
      setRule((currentRule) => ({ ...currentRule, resourceId: current?.resources?.some((resource) => resource.id === currentRule.resourceId) ? currentRule.resourceId : current?.resources?.[0]?.id || '' }))
      setComponent((currentComponent) => ({ ...currentComponent, childCatalogItemId: componentPayload.data.candidates?.some((candidate: Candidate) => candidate.id === currentComponent.childCatalogItemId) ? currentComponent.childCatalogItemId : componentPayload.data.candidates?.[0]?.id || '', childVariantId:'' }))
  ...
  async function savePolicy() {
    ...
    try {
      ...
      const allowedTerms = policy.serviceAreas.split(',').map((entry) => entry.trim()).filter(Boolean)
      await request(`/api/vendor/catalog/${encodeURIComponent(selected.id)}`, 'PATCH', {
        operatingTimezone: policy.timezone.trim() || null,
        ...
        serviceAreaPolicy: allowedTerms.length
          ? { mode:'text_allowlist', required:policy.serviceAreaRequired, allowedTerms }
          : { mode:'none', required:false, allowedTerms:[] },
      })
      setMessage('Booking policy saved to the deterministic availability engine.')
      await loadCatalog()
