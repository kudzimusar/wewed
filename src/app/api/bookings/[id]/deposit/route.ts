...
import { syncBookingDeposit } from '@/lib/booking-governance'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await syncBookingDeposit({
      bookingId: id,
    ...
    console.error('[BOOKING DEPOSIT POST] error:', error)
    return NextResponse.json({ success: false, error: 'Unable to confirm recorded deposit.' }, { status: 500 })
  }
}