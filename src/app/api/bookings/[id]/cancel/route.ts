...
import { cancelBookingGoverned } from '@/lib/booking-governance'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await cancelBookingGoverned({
      bookingId: id,
    ...
    return NextResponse.json({ success: false, error: 'Unable to cancel booking.' }, { status: 500 })
  }
}
