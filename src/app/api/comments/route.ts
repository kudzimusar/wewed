...
/* ============================================================
   ...
   GET  ?targetType=media&targetId=xxx&weddingSlug=your-wedding-slug
...
export async function GET(request: NextRequest) {
  try {
    const weddingSlug = request.nextUrl.searchParams.get('weddingSlug')?.trim() || request.nextUrl.searchParams.get('slug')?.trim()
    ...
    if (!weddingSlug && !weddingId) {
      return NextResponse.json(
        { success: false, error: 'weddingSlug or weddingId is required.' },
        { status: 400 }
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    // 1. Parse + validate the comment body and wedding context
    const body = await request.json().catch(() => ({}))
    const weddingSlug = (body?.weddingSlug || body?.slug || request.nextUrl.searchParams.get('weddingSlug') || request.nextUrl.searchParams.get('slug'))?.trim()
    const weddingId = (body?.weddingId || request.nextUrl.searchParams.get('weddingId'))?.trim()
    ...
    // 3. Ensure the user has a UserProfile
