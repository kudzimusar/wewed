...
export function generateStaticParams() {
  return getPublicDocuments('help').map(({ slug }) => ({ slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params
  const document = getPublicDocument('help', slug)
  if (!document) return {}
  return { title: `${document.title} | Wewed`, description: document.summary, alternates: { canonical: `/help/${slug}` } }
}
