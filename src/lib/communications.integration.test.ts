  ...
  beforeAll(async () => {
    ...
    await db.$executeRaw(Prisma.sql`
      INSERT INTO public."BusinessAccount"
        ("id", "name", "slug", "type", "status", "onboardingStatus", "subscriptionStatus")
      VALUES
        (${PLANNER_BUSINESS_ID}, 'Planner Business Name', 'communications-integration-planner-business', 'planning_company', 'active', 'complete', 'free'),
        (${SECOND_PLANNER_BUSINESS_ID}, 'Second Planner Business', 'communications-integration-second-planner-business', 'planning_company', 'active', 'complete', 'free')
    `)
    await db.$executeRaw(Prisma.sql`
