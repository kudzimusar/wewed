...
export async function readWeddingDayGuestContext(request: NextRequest) {
  ...
  const eligibility = await guestEligibility(session.weddingId, session.guestId)
  if (!eligibility) return null
  const credentials = await db.$queryRawUnsafe<CredentialRow[]>(
    `SELECT * FROM public."WeddingPassCredential"
      WHERE "weddingId" = $1 AND "guestId" = $2 AND "revokedAt" IS NULL
      ORDER BY "issuedAt" DESC LIMIT 1`,
    session.weddingId,
    session.guestId,
  )
  const announcements = await activeAnnouncements(session.weddingId, 'guest')
  return {
    actorRole: 'guest' as const,
    guest: {
      id: eligibility.guestId,
      name: eligibility.guestName,
      attending: eligibility.attending,
      tableNumber: eligibility.tableNumber,
      household: householdMembers(eligibility),
    },
    wedding: {
      id: eligibility.weddingId,
      slug: eligibility.weddingSlug,
      title: eligibility.weddingTitle,
      date: eligibility.weddingDate,
...
export async function guestPassForRequest(request: NextRequest) {
  const session = readWeddingGuestSession(request)
  if (!session) return null
  return ensureWeddingPassCredential({
