...
const liveClosure = read('docs/NOTEBOOK_LIVE_TRANSCRIPTION_CLOSURE_2026-08-20.md')
...
const liveBridge = read('src/components/notebook/notebook-live-transcription-bridge.tsx')
...
const transcriptionChunkRoute = read('src/app/api/notebook/[id]/transcription-chunks/route.ts')
const transcriptionReadinessRoute = read('src/app/api/notebook/transcription-readiness/route.ts')
...
describe('WW-NOTEBOOK-OPS-2026-08-19-01', () => {
  ...
  test('preserves the canonical recording and selects a provider-compatible transcription path', () => {
    const uploadIndex = recordingRoute.indexOf('await uploadRecording')
    const directTranscribeIndex = recordingRoute.indexOf('await transcribeRecording')
    expect(uploadIndex).toBeGreaterThan(-1)
    expect(directTranscribeIndex).toBeGreaterThan(uploadIndex)
    ...
    expect(transcriptionConfig).toContain('ZAI_API_KEY')
    expect(transcriptionConfig).toContain('glm-asr-2512')
    ...
    expect(media).toContain('TRANSCRIPTION_REQUIRES_LIVE_CHUNKS')
    expect(clarity).toContain('Record & transcribe')
    expect(clarity).toContain('preserve the private audio safely')
  })

  test('captures long Z.AI meetings as bounded private WAV chunks while retaining the original WebM', () => {
    expect(liveBridge).toContain('LiveNotebookCapture')
    expect(liveBridge).toContain('encodeMonoPcm16Wav')
    expect(liveBridge).toContain('notebookLiveChunkSampleLimit')
    expect(liveBridge).toContain('/transcription-chunks')
    expect(liveBridge).toContain("action: 'attach-transcript'")
    expect(liveBridge).toContain('window.MediaRecorder = PatchedMediaRecorder')
    expect(workspace).toContain("new File([blob], `meeting-${Date.now()}.webm`")
    expect(plannerPage).toContain('<NotebookLiveTranscriptionBridge mode={mode} />')
    expect(transcriptionChunkRoute).toContain('requireNotebookActor')
    expect(transcriptionChunkRoute).toContain("scope: 'notebook-live-transcription'")
    expect(transcriptionChunkRoute).toContain('transcribeNotebookAudioChunk')
    expect(transcriptionReadinessRoute).toContain("config.requestShape === 'zai' ? 'live-chunks' : 'direct'")
    expect(recordingActionRoute).toContain("action === 'attach-transcript'")
    expect(recordingActionRoute).toContain('attachTranscript')
    expect(media).toContain("'LIVE_TRANSCRIPTION_CHUNK'")
    expect(media).toContain("'TRANSCRIPT_ATTACHED'")
    expect(liveClosure).toContain('original browser WebM recording as the canonical private audio evidence')
  })
  ...
  test('records the operational standard without weakening Notebook or financial authority', () => {
    ...
    expect(liveClosure).toContain('No schema migration and no authorization broadening.')
  })
})
