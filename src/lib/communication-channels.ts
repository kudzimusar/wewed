...
export type CommunicationDeliveryStatus = 'QUEUED' | 'PROCESSING' | 'SENT' | 'DELIVERED' | 'FAILED' | 'SKIPPED'
...
export async function listCommunicationChannelSettings(actor: CommunicationActor) {
  const rows = await db.$queryRaw<EndpointRow[]>(Prisma.sql`
    SELECT
      ...
      endpoint."createdAt",
      endpoint."updatedAt",
      COALESCE(preference."enabled", false) AS "enabled"
  ...
  return {
    ...
    preferences: EXTERNAL_CHANNELS.map((channel) => ({
      channel,
      enabled: preferenceMap.get(channel) ?? false,
    })),
  }
}
...
export async function registerCommunicationEndpoint(
  ...
  const rows = await db.$queryRaw<Array<{ id: string; status: CommunicationEndpointStatus }>>(Prisma.sql`
    ...
    VALUES (
      ${id}, ${actor.userId}, ${input.channel}, ${endpoint.address}, ${endpoint.normalizedAddress},
...
export function buildResendEmailRequest(input: ClaimedDeliveryRow) {
  const from = process.env.WEWED_COMMUNICATION_FROM?.trim()
  ...
  const subject = `${input.senderName} sent you a message on Wewed`
  return {
    ...
    body: {
      ...
      ...(process.env.WEWED_COMMUNICATION_REPLY_TO?.trim()
        ? { reply_to: process.env.WEWED_COMMUNICATION_REPLY_TO.trim() }
        : {}),
      tags: [{ name: 'source', value: 'wewed-communications' }],
    },
  }
}
  ...
  const body = template
    ? {
        ...
        template: {
          ...
          components: [{
            type: 'body',
            parameters: [{ type: 'text', text: input.body.slice(0, 1000) }],
          }],
        },
      }
    : {
...
function buildGenericGatewayRequest(
  channel: 'SMS' | 'PUSH',
  input: ClaimedDeliveryRow,
  ...
  return {
    ...
    body: channel === 'SMS'
      ...
      : {
          ...
          url: `${applicationBaseUrl()}/messages?conversation=${encodeURIComponent(input.conversationId)}`,
          messageId: input.messageId,
          deliveryId: input.id,
        },
  }
...
async function dispatchTransport(input: ClaimedDeliveryRow): Promise<TransportResult> {
  if (input.channel === 'EMAIL') {
    return sendJson('resend', buildResendEmailRequest(input), (payload) => {
  ...
  const request = buildGenericGatewayRequest(input.channel, input)
  return sendJson(input.channel === 'SMS' ? 'sms-gateway' : 'push-gateway', request, (payload) => {
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return undefined
    const id = (payload as Record<string, unknown>).id
    return typeof id === 'string' ? id : undefined
  })
  ...
  return db.$transaction(async (tx) => {
    const candidates = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      ...
      WHERE delivery."status" = 'QUEUED'
        AND delivery."channel" IN ('EMAIL', 'WHATSAPP', 'SMS', 'PUSH')
  ...
  if (result.unavailable) {
    await db.$executeRaw(Prisma.sql`
      ...
      SET
        "status" = 'SKIPPED',
        "provider" = ${result.provider},
        "errorCode" = ${result.errorCode ?? 'TRANSPORT_NOT_CONFIGURED'},
        "nextAttemptAt" = NULL,
        "updatedAt" = ${now}
      WHERE "id" = ${delivery.id}
        AND "status" = 'PROCESSING'
...
export async function processQueuedCommunicationDeliveries(limit = 20) {
  ...
  for (let index = 0; index < safeLimit; index += 1) {
    ...
    results.push({
      ...
      status: result.ok ? 'SENT' : result.unavailable ? 'SKIPPED' : result.retriable && delivery.attemptCount < delivery.maxAttempts ? 'QUEUED' : 'FAILED',
    })
  }
...
function providerEventMetadata(value: unknown): Record<string, unknown> {
  ...
  for (const key of ['reason', 'code', 'template', 'providerStatus']) {
    const item = source[key]
    if ((typeof item === 'string' && item.length <= 200) || typeof item === 'number' || typeof item === 'boolean') {
      result[key] = item
    }
  }
  return result
...
export async function applyCommunicationProviderStatus(input: {
  provider: string
  channel: ExternalCommunicationChannel
  ...
  return db.$transaction(async (tx) => {
    const inserted = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      INSERT INTO wewed_communications."CommunicationProviderEvent"
        ("id", "provider", "channel", "providerEventId", "direction", "eventType", "status", "metadata", "createdAt")
      VALUES (
        ${randomUUID()}, ${input.provider}, ${input.channel}, ${input.providerEventId}, 'STATUS',
        ${input.status.toLowerCase()}, 'RECEIVED', ${JSON.stringify(providerEventMetadata(input.metadata))}::jsonb, ${now}
      )
    ...
    const deliveries = await tx.$queryRaw<ProviderMessageRow[]>(Prisma.sql`
      SELECT "id", "messageId", "recipientUserId", "channel"
    ...
    await tx.$executeRaw(Prisma.sql`
      ...
      SET
        ...
        "errorCode" = ${input.status === 'FAILED' ? 'PROVIDER_REPORTED_FAILURE' : null},
        "updatedAt" = ${now}
      WHERE "id" = ${delivery.id}
    `)
    await tx.$executeRaw(Prisma.sql`
      UPDATE wewed_communications."CommunicationProviderEvent"
      SET "status" = 'PROCESSED', "deliveryId" = ${delivery.id}, "messageId" = ${delivery.messageId}, "processedAt" = ${now}
      WHERE "id" = ${inserted[0].id}
    `)
...
export async function ingestInboundCommunicationReply(input: {
  provider: string
  channel: ExternalCommunicationChannel
  providerEventId: string
  fromAddress: string
  ...
  const resolved = await db.$queryRaw<Array<{
    deliveryId: string
    conversationId: string
    userId: string
    email: string
    ...
    JOIN wewed_communications."CommunicationEndpoint" endpoint
      ON endpoint."id" = delivery."endpointId"
     AND endpoint."userId" = delivery."recipientUserId"
     AND endpoint."status" = 'VERIFIED'
    ...
    WHERE delivery."provider" = ${input.provider}
      AND delivery."providerMessageId" = ${input.replyToProviderMessageId}
      AND delivery."channel" = ${input.channel}
  ...
  if (!target || !isDashboardRole(target.role)) {
    throw new CommunicationError('Inbound reply could not be associated with a verified Wewed participant.', 404)
  }
  ...
  const inserted = await db.$queryRaw<Array<{ id: string }>>(Prisma.sql`
    ...
    VALUES (
      ${eventId}, ${input.provider}, ${input.channel}, ${input.providerEventId}, 'INBOUND', 'reply', 'RECEIVED',
      ${target.deliveryId}, ${JSON.stringify({ bodySha256: eventHash })}::jsonb
    )
    ON CONFLICT ("provider", "providerEventId") DO NOTHING
    RETURNING "id"
  `)
