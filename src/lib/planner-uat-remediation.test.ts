...
describe('planner UAT remediation contracts', () => {
  test('planner workspace treats module fetches as independently recoverable', async () => {
    ...
    expect(workspace).toContain('refreshControllerRef.current?.abort()')
    expect(workspace).toContain('signal: controller.signal')
    expect(workspace).toContain("console.warn('[PLANNER WORKSPACE CLIENT] refresh failed'")
    expect(portal).toContain("[PLANNER PORTAL CLIENT] session refresh failed")
    expect(portal).toContain("response.json().catch(() => null)")
    expect(portal).toContain('activeController?.abort()')
    expect(portal).toContain("signal: controller.signal")
    expect(portal).toContain("console.warn('[PLANNER PORTAL CLIENT] session refresh failed'")
