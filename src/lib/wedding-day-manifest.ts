...
/**
 * Builds the field-operations manifest consumed by native usher clients.
 *
 * The payload deliberately carries only admission data needed at the gate:
 * public signing keys, pass-to-guest mapping, the exact household attendee keys,
 * current admission state, and revocation/expiry metadata. It never contains a
 * signing private key or a reusable application session token.
 */
export async function signedNativeWeddingDayManifest(weddingId: string) {
  ...
  return {
    rootKeyId:
      process.env.WEDDING_DAY_ROOT_KEY_ID?.trim() || 'wewed-root-isolated-v1',
    algorithm: WW2_ALGORITHM,
    payload,
    canonicalPayload,
