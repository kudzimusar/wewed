...
export const POLICY_EFFECTIVE_DATE = 'September 7, 2026'
...
export const PUBLIC_DOCUMENTS: PublicDocument[] = [
  ...
  legal('privacy', 'Privacy Policy', 'How Wewed handles account, wedding, guest, vendor, planner, usage and support data.', [
    { heading: 'Information we process', bullets: ['Account and authentication information, including name, email address, role, account status and sign-in records.', 'Wedding/project details, tasks, schedules, budgets, seating, preferences, notes and workspace activity.', 'Guest and contributor information supplied by authorised users, which may include names, contact details, addresses, relationships, RSVP status, meal choices, dietary or allergy information, accessibility needs, and cultural or religious requirements relevant to the celebration.', 'Messages, comments, enquiries, announcements and other communications sent through Wewed.', 'Photos, video, audio or voice input and transcripts, contracts, receipts, spreadsheets and other files a user chooses to upload or connect.', 'Planner and vendor business/profile information, service details, availability, enquiries, reviews and marketplace activity.', 'Subscription, transaction and billing metadata; payment-card processing is handled by the payment processor rather than stored as raw card data by Wewed.', 'Device, browser, push-subscription, security, diagnostic and usage information needed to operate and protect the service.', 'Prompts, generated responses and the minimum relevant workspace context when an AI feature is used.', 'Integration data, such as Google Sheets identifiers and limited access tokens, when a user deliberately connects a supported service.'] },
    ...
    { heading: 'Sharing and processors', paragraphs: ['We share information with authorised workspace participants according to product permissions and with processors that help run Wewed. Depending on the feature and configuration, these may include Vercel for application hosting, Supabase for authentication, database and storage, Stripe for web billing, Resend for email delivery, Meta for configured WhatsApp delivery, Google for connected Sheets or Gemini features, and configured AI providers such as Groq or Z.ai. Each provider receives only the data needed for the service it performs.', 'We may disclose information where required by law or where reasonably necessary to protect users, investigate abuse or secure the service. We do not sell personal information. We do not treat a user uploading private wedding information as permission to make it public. Visibility is governed by the feature, workspace permissions and choices made by authorised users.'] },
    { heading: 'Security and international processing', paragraphs: ['Wewed uses HTTPS in production, authenticated sessions, role and wedding-scoped authorisation, access-controlled storage, audit records and restricted administrative access to protect information. No internet service can guarantee absolute security. Report suspected compromise to security@wewed.pro.', 'Providers may process information in countries other than the user’s own. Wewed uses appropriate contractual and technical safeguards where applicable and limits processor access to the operational purpose.'] },
    { heading: 'Retention and choices', paragraphs: ['We retain information for as long as needed to provide the service, maintain legitimate business and security records, resolve disputes and meet legal obligations. You can initiate account deletion at https://wewed.pro/account-deletion or from Settings inside the authenticated product. Verified requests are normally completed within 30 days; removal from encrypted backups may take up to 90 days as backups cycle out.', 'Some minimum records may be retained where required for security, fraud prevention, financial reporting, legal obligations, disputes or the rights of other wedding participants. Privacy rights differ by location. Wewed will honor rights required by applicable law after appropriate identity verification.'] },
    { heading: 'Contact', paragraphs: ['For privacy questions or requests, email privacy@wewed.pro. For product support, email support@wewed.pro. Do not send passwords, one-time codes or unnecessary guest information.'] },
  ]),
