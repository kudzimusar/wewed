...
describe('digital invitation card delivery', () => {
  ...
  test('the shared dashboard API proxy prevents authenticated responses from shared caching', () => {
    ...
    expect(proxy).toContain("response.headers.set('Vary', 'Cookie, Authorization')")
    expect(proxy).toContain('const session = protectedPlannerPage ? cookieSession : readAppSession(request)')
