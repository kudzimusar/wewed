...
describe('Wedding Architect candidate pricing', () => {
  ...
  test('supports approved fixed package compound overage without parsing pricing labels', () => {
    const result = priceWeddingArchitectCandidate({
      weddingBudgetCents: 2_000_000,
      quantityContext,
      variant: {
        ...
        packageName: 'Guest rooms', packagePriceCents: 100_000, packagePricingUnit: 'per package',
  ...
  test('rejects a package base price expressed as an unstructured variable unit', () => {
    const result = priceWeddingArchitectCandidate({
      weddingBudgetCents: 2_000_000,
      quantityContext,
      variant: {
        providerId: 'provider-2', businessAccountId: 'business-2', offeringId: 'food-1', packageId: 'package-variable', category: 'catering', currency: 'USD',
        pricingVisibility: 'range', startingPriceCents: 0,
        packageName: 'Dinner package', packagePriceCents: 2_500, packagePricingUnit: 'per guest',
        packageCommercialTerms: { taxIncluded: true, depositType: 'none' },
      },
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.reasons.join(' ')).toContain('Package base price is not marked as a fixed/package total')
  })
