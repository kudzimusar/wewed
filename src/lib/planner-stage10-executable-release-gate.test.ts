...
describe('Stage 10 executable planner release gate', () => {
  ...
  test('external notification delivery authorizes, deep-links and applies policy before consuming its batch', async () => {
    ...
    expect(push).toContain('return `vapid t=${signingInput}.${encodeBase64Url(signature)}, k=${config.publicKey}`')
    expect(push).toContain('Authorization: vapidAuthorization(subscription.endpoint, config)')
  ...
  test('notification lifecycle and channel settings enforce multi-device semantics', async () => {
    ...
    expect(capabilities).toContain('getCommunicationChannelActivation(userId)')
    expect(capabilities).toContain('activation.EMAIL.transportConfigured')
    expect(capabilities).toContain('activation.EMAIL.endpointVerified')
    expect(capabilities).toContain('activation.EMAIL.preferenceEnabled')
    expect(capabilities).toContain('activation.PUSH.transportConfigured')
    expect(capabilities).toContain('activation.PUSH.activeDeviceCount')
    expect(capabilities).toContain('activation.PUSH.canEnable')
