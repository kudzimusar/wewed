...
describe('premium digital invitation experience', () => {
  test('Card 1 is implemented as a real interactive tri-fold with correct marriage wording', () => {
  ...
  test('motion is interactive and honours reduced-motion users without video', () => {
    const experience = source(
      'src/components/wedding/invitation-experience/premium-invitation-experience.tsx',
    )
  ...
  test('personal invitations show the premium experience only after scoped guest access', () => {
  ...
  test('personal RSVP remains guest-session scoped and physical invitation identity stays separate', () => {
    ...
    expect(physical).toContain('WEDDING_SHARED_INVITATION_COOKIE')
