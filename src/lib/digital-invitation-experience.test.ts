...
describe('premium digital invitation experience', () => {
  ...
  test('the interactive invitation surface connects guest actions instead of becoming a decorative dead end', () => {
    ...
    expect(experience).toContain("visitCoupleWebsite('#registry')")
    expect(experience).toContain('wewed:skip-invitation-once:')
    expect(experience).toContain('window.location.assign(`${path}${anchor}`)')
    expect(experience).toContain('Visit Couple Website')
    expect(experience).toContain('data-testid="invitation-footer-actions"')
    expect(experience).toContain('data-testid="invitation-guest-personalization"')
    expect(experience).not.toContain('Back to Wewed Couple Site')
    ...
    // The global navigator remains mounted for physical invitation and other
    // contribution entry paths, while Ivory itself now navigates deterministically.
  ...
  test('children policy is wedding-scoped, backwards compatible, and enforced by the guest-session API', () => {
    ...
    // Master plan Phase 9 — the actual enforcement now lives in the shared
    // `applyGuestRsvpUpdate` operation, reused by /api/rsvp too (see guest-rsvp-mutation.test.ts
    // for its own executable disposable-DB coverage); the guest-session route still surfaces the
    // resulting `CHILDREN_NOT_ALLOWED` code itself, so that assertion stays pinned to this file.
    const guestRsvpMutation = source('src/lib/guest-rsvp-mutation.ts')
    const rsvp = source(
    ...
    expect(guestApi).toContain("applyGuestRsvpUpdate")
    expect(guestApi).toContain("code: 'CHILDREN_NOT_ALLOWED'")
    expect(guestRsvpMutation).toContain("type ChildrenPolicy = 'welcome' | 'adults_only'")
    expect(guestRsvpMutation).toContain("code: 'CHILDREN_NOT_ALLOWED'")
    expect(guestRsvpMutation).toContain('delete data.kidsCount')
  ...
  test('personal invitations show the premium experience only after scoped guest access and carry their palette into details', () => {
    ...
    expect(page).toContain("const explicitInvitationRequested = query.invitation === '1'")
    expect(page).toContain('guestInvitationResolution?.allowed')
    expect(page).toContain("guestInvitationResolution.accessKind === 'invited_guest'")
    expect(page).toContain('guestInvitationResolution.guest')
