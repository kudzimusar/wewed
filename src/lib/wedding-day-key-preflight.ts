...
export function weddingDayKeyPreflight(
  ...
  checks.push({
    ...
    // The key *identifier* is not a secret — it is published in the gate manifest.
  ...
  // The two keys must be independent. One key signing both passes and the manifest that lists
  // them would mean a single compromise forges both the credential and its own attestation.
