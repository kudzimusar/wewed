...
(
  ...
  # Expo's --localhost path passes the hostname "localhost" to Node's HTTP
  # listener. GitHub's Ubuntu image resolves localhost to ::1 first, while adb
  # reverse forwards the device's localhost:8081 connection to IPv4 localhost
  # on the runner. Force Node's resolver order so Metro binds 127.0.0.1 and the
  # compiled debug APK can actually request its JavaScript bundle through adb.
  exec node --dns-result-order=ipv4first ./node_modules/expo/bin/cli start --localhost --clear
) > "$METRO_LOG" 2>&1 &
metro_pid=$!
...
for attempt in $(seq 1 120); do
  # Probe IPv4 explicitly: this is the same host-side endpoint adb reverse must
  # reach for the Android app to load its development bundle.
  ...
  if [[ "$attempt" -eq 120 ]]; then
    echo 'Metro did not become ready on IPv4 localhost.' >&2
    cat "$METRO_LOG" || true
